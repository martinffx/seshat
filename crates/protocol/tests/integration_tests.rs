...
use seshat_protocol::{RespCodec, RespCommand, RespValue};
use std::io;
...
async fn test_parse_encode_roundtrip_all_commands() {
    ...
    for (value, expected_cmd) in commands {
        ...
        let mut buf = BytesMut::new();
        seshat_protocol::RespEncoder::encode(&value, &mut buf).unwrap();

        // Parse back
        let mut parser = seshat_protocol::RespParser::new();
